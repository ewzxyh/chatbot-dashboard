...
export class ResponseTimesComponent implements OnInit {
  ...
  getCurrentProject() {
    ...
    .subscribe((project) => {
      this.logger.log('[ANALYTICS - RESPONSETIMES] - PRJCT FROM SUBSCRIPTION TO AUTH SERV  ', project)
      if (project) {
  ...
  findCurrentProjectAmongAll(projectId: string) {
    ...
    this.projectService.getProjects().subscribe((projects: any) => {
      this.logger.log('[ANALYTICS - RESPONSETIMES] - GET PROJECTS - projects ', projects);  
      const current_selected_prjct = projects.find(prj => prj.id_project.id === projectId);
      this.logger.log('[ANALYTICS - RESPONSETIMES] - GET PROJECTS - current_selected_prjct ', current_selected_prjct);
      if (current_selected_prjct && current_selected_prjct.id_project.profile) {
        ...
        if (projectProfile && projectProfile.customization) {
            ...
            this.logger.log('[ANALYTICS - RESPONSETIMES] - customization', customization);
      ...
      this.logger.error('[ANALYTICS - RESPONSETIMES] - GET PROJECTS - ERROR: ', error);
    }, () => {
      this.logger.log('[ANALYTICS - RESPONSETIMES] - GET PROJECTS * COMPLETE * ');
    });
  }
