...
export class ActivateAppsumoProductComponent extends WidgetSetUpBaseComponent implements OnInit {
  ...
  constructor(
    ...
    this.contactSalesEmail = "redacted@example.invalid"
  }
  ...
  contactUs() {
    ...
    if (this.USER_ROLE === 'owner') {
      window.open(`mailto:redacted@example.invalid?subject=Support request for AppSumo license id project ${this.new_project._id}` );
    } else {
      this.presentModalOnlyOwnerCanManageTheAccountPlan()
    }
  }
